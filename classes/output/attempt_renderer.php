...
class attempt_renderer extends \plugin_renderer_base
{
    ...
    public function add_edit_page_links($solo, $latestattempt, $thisstep, $cm, $context)
    {
        ...
        if ($stepdata->percentcomplete > 100) {
            $stepdata->percentcomplete = 100;
        }
    ...
    function show_summarypassageandstats($moduleinstance, $attempt, $aidata, $stats, $autotranscriptready, $selftranscribe)
    {
        ...
        if (
            $moduleinstance->step1 == constants::M_STEP_TRANSCRIBE ||
        ...
        if ($tdata['spellingerrors']) {
            $tdata['hasspellingerrors'] = true;
        }
        if ($tdata['grammarerrors']) {
            $tdata['hasgrammarerrors'] = true;
        }
        if ($selftranscribe) {
            $tdata['selftranscribe'] = true;
        }
        if ($moduleinstance->showgrammar) {
            $tdata['showgrammar'] = true;
        }
        if ($moduleinstance->showspelling) {
            $tdata['showspelling'] = true;
        }

        // If you have no transcript then it will error on render, so we use a space by default.
        // It should never really be blank however, and theuser arrived in a strange way probably.
        // This just avoids an ugly error.
    ...
    function show_myreports($moduleinstance, $cm)
    {
        ...
        $myprogress = new \single_button(
            new \moodle_url(
                constants::M_URL . '/myreports.php',
                ['report' => 'myprogress', 'id' => $cm->id, 'n' => $moduleinstance->id, 'format' => 'linechart']
            ),
            get_string('myprogressreport', constants::M_COMPONENT),
            'get'
        );
        ...
        $myattempts = new \single_button(
            ...
            get_string('myattempts', constants::M_COMPONENT),
            'get'
        );
