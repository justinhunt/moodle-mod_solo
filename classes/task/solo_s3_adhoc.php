...
class solo_s3_adhoc extends \core\task\adhoc_task
{
    ...
    public function execute()
    {
        ...
        $activity = $DB->get_record(constants::M_TABLE, ['id' => $cd->activityid]);
        if (!utils::can_transcribe($activity)) {
            $this->do_forever_fail('This activity does not support transcription', $trace);
            return;
        }

        $attempt = $DB->get_record(constants::M_ATTEMPTSTABLE, ['id' => $cd->attemptid]);
        if ($attempt) {
    ...
    protected function do_retry($reason, $trace, $customdata)
    {
        ...
        } else if ($customdata->taskcreationtime + (MINSECS * 15) < time()) {
            // 15 minute delay.
            $delay = (MINSECS * 15);
        } else {
            // 30 second delay.
        ...
        $s3task->set_next_run_time(time() + $delay);
        // Queue it.
