...
class aitranscript {
    ...
    function __construct($attemptid, $modulecontextid=0, $passage=false, $transcript=false, $jsontranscript=false) {
        ...
        $this->attemptdata = $DB->get_record(constants::M_ATTEMPTSTABLE, ['id' => $attemptid]);
        if ($this->attemptdata) {
            $this->activitydata = $DB->get_record(constants::M_TABLE,
                    ['id' => $this->attemptdata->{constants::M_AI_PARENTFIELDNAME}]);
            $record = $DB->get_record(constants::M_AITABLE, ['attemptid' => $attemptid]);
            if ($record) {
                $this->recordid = $record->id;
                $this->aidata = $record;
            } else if ($passage !== false && $jsontranscript && $transcript !== null && $transcript !== false) {
